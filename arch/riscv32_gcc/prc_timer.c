 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: prc_timer.c 264 2019-01-10 18:03:26Z roi $
 */
...
{
	CLOCK mtimecmp = prc_current_timer_value() + TO_CLOCK(TIC_NUME, TIC_DENO);
...
prc_timer_initialize(intptr_t exinf)
{
	 ...
	 */
	mtimecmp = prc_current_timer_value() + TO_CLOCK(TIC_NUME, TIC_DENO);
	sil_wrw_mem((uint32_t *)(TADR_CLIC_BASE+TOFF_CLINT_MTIMECMP+4), (uint32_t)(mtimecmp >> 32));
	sil_wrw_mem((uint32_t *)(TADR_CLIC_BASE+TOFF_CLINT_MTIMECMP), (uint32_t)mtimecmp);
...
prc_timer_handler(void)
{
	...
	kernel_mie &= ~MIP_MTIP;
	mtimecmp = prc_terget_timer_value();
	do{
		...
		molecule += REMAINDER(TIC_NUME, TIC_DENO);
		difference = mtimecmp - prc_current_timer_value();
	}while(difference <= 0);
	sil_wrw_mem((uint32_t *)(TADR_CLIC_BASE+TOFF_CLINT_MTIMECMP+4), (uint32_t)(mtimecmp >> 32));
	sil_wrw_mem((uint32_t *)(TADR_CLIC_BASE+TOFF_CLINT_MTIMECMP), (uint32_t)mtimecmp);
