 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: prc_config.c 1430 2019-01-10 18:02:55Z roi $
 */

/*
 *		プロセッサ依存モジュール（RISCV/32用）
 */
...
{
	uint32_t mstatus = *(((uint32_t*)p_excinf) + P_EXCINF_OFFSET_MSTATUS);
	uint32_t pc      = *(((uint32_t*)p_excinf) + P_EXCINF_OFFSET_PC);
	uint32_t excno   = mcause & MCAUSE_CAUSE;

	excno += (mcause & MCAUSE_INT) >> 27;
	syslog(LOG_EMERG, "\nUnregistered Interrupt occurs.");
	syslog(LOG_EMERG, "Excno = 0x%02X, PC = 0x%08X, mstatus = 0x%08X, p_excinf = 0x%08X",
...
prc_initialize(void)
{
	...
	if(read_csr(misa) & (1 << ('F' - 'A'))){
		write_csr(mstatus, MSTATUS_FS);	/* FPU設定、但し割込みはサポートしない */
...
handle_trap(unsigned long mcause, void *p_excinf)
{
	...
	if(intno < TMAX_MACHNE_INTNO){
		...
		syslog_1(LOG_EMERG, "Irrigal machine Exception mcause = %08X", (int)mcause);
	}
	if(--inest_lvl == 0)
		write_csr(mtvec, &trap_entry);
	return inest_lvl;
...
{
	uint32_t *excsp = (uint32_t *) p_excinf;

	syslog_0(LOG_ERROR, "CPU Exception Information:");
	syslog_4(LOG_ERROR, "mstatus = %08x PC = %08x mcause = %08x STACK = %08x",
				*((excsp + P_EXCINF_OFFSET_MSTATUS)), 
				*((excsp + P_EXCINF_OFFSET_PC)),
				*((excsp + P_EXCINF_OFFSET_MCAUSE)),
				((uint32_t)excsp + 19 * 4));
	syslog_1(LOG_ERROR, "p_excinf = %08x", (uint32_t)excsp);
}
